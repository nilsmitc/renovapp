<script lang="ts">
	...
	const changelogHtml = $derived.by(() => {
		...
		return data.changelog
			...
			.replace(/^## (.+)$/gm, '<h3 class="font-semibold text-stone-800 dark:text-stone-200 mt-3 first:mt-0">$1</h3>')
			.replace(/^# .+\n?/gm, '')
			.replace(/^- (.+)$/gm, '<li class="ml-4 list-disc text-stone-600 dark:text-stone-400">$1</li>')
...
<div class="space-y-6">
	...
	{#if data.updating}
		<div class="card p-6 space-y-4">
			<div class="flex items-center gap-3">
				<svg class="w-5 h-5 text-primary-500 dark:text-primary-400 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
				<div>
					<div class="text-sm font-semibold text-stone-900 dark:text-stone-100">Update wird installiert...</div>
					<div class="text-xs text-stone-500 dark:text-stone-400 mt-1">Der Server startet automatisch neu. Diese Seite wird in wenigen Sekunden neu geladen.</div>
				</div>
			</div>
		</div>
	{:else}
		<div class="card p-6 space-y-3">
			...
			{:else if updateStatus?.updateVerfuegbar}
				<div class="alert-warning space-y-3">
					<div class="flex items-center gap-2">
						...
						<span class="text-sm font-semibold text-amber-800 dark:text-amber-200">{updateStatus.commits?.length ?? 0} neue Updates verfügbar</span>
					</div>
					{#if updateStatus.commits && updateStatus.commits.length > 0}
						<ul class="text-xs text-stone-700 dark:text-stone-300 space-y-1 ml-7">
							{#each updateStatus.commits as commit}
								<li>
									<span class="font-mono text-stone-400 dark:text-stone-500">{commit.hash}</span>
									<span class="text-stone-400 dark:text-stone-500 mx-1">{commit.datum}</span>
									{commit.nachricht}
								</li>
							{/each}
						</ul>
					...
					<form method="POST" action="?/update" use:enhance={() => {
						...
						<button type="submit" class="btn-primary inline-flex items-center gap-1.5" disabled={updateLaeuft}>
							{#if updateLaeuft}
					...
					<p class="text-xs text-stone-500 dark:text-stone-400 ml-7">Der Server wird kurz neu gestartet. Deine Daten bleiben erhalten.</p>
				</div>
			{:else if updateStatus && !updateStatus.updateVerfuegbar}
				<div class="alert-success flex items-center gap-2">
					...
					<span class="text-green-500 dark:text-green-400 font-mono text-xs ml-1">({updateStatus.aktuellerCommit})</span>
				</div>
			{/if}
			...
			{#if data.changelog}
				<details class="group">
					<summary class="flex items-center gap-1.5 text-sm text-stone-500 dark:text-stone-400 hover:text-stone-700 dark:hover:text-stone-200 cursor-pointer select-none transition-colors">
					...
					<div class="mt-2 ml-5.5 text-sm text-stone-600 dark:text-stone-400 space-y-3 changelog-content">
						{@html changelogHtml}
					</div>
				</details>
			{/if}
	...
	<div class="card p-6 space-y-3">
		<h2 class="flex items-center gap-2 text-lg font-semibold text-stone-800 dark:text-stone-200">
			<svg class="w-5 h-5 text-stone-500 dark:text-stone-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3" /></svg>
			Backup exportieren
		</h2>
	...
	<div class="card p-6 space-y-4">
		<h2 class="flex items-center gap-2 text-lg font-semibold text-stone-800 dark:text-stone-200">
			<svg class="w-5 h-5 text-stone-500 dark:text-stone-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
			Belege-Export (BAFA / Energieberater)
		</h2>
		<p class="text-sm text-stone-600 dark:text-stone-400">
			Konfiguration für den Belege-Export auf der <a href="/belege" class="text-primary-600 dark:text-primary-400 hover:underline">Belege-Seite</a>.
		...
		<div class="space-y-3">
			<div>
				<label class="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">E-Mail Energieberater</label>
				<input
					type="email"
			...
			<div>
				<label class="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Thunderbird-Pfad</label>
				<input
					type="text"
				...
				/>
				<p class="text-xs text-stone-400 dark:text-stone-500 mt-1">Leer lassen für Standardpfad ("thunderbird")</p>
			</div>
			<div>
				<label class="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">E-Mail Betreff</label>
				<input
					type="text"
					...
					class="input-base w-full max-w-sm"
				/>
				<p class="text-xs text-stone-400 dark:text-stone-500 mt-1">Leer lassen für Standardbetreff</p>
			</div>
			<button
	...
	<div class="card p-6 space-y-3">
		<h2 class="flex items-center gap-2 text-lg font-semibold text-stone-800 dark:text-stone-200">
			<svg class="w-5 h-5 text-stone-500 dark:text-stone-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" /></svg>
			Backup importieren
		</h2>
		<p class="text-sm text-stone-600 dark:text-stone-400">
			Stellt ein zuvor erstelltes Backup wieder her. <strong class="text-red-700 dark:text-red-400">Alle
		...
		<form method="POST" action="?/import" enctype="multipart/form-data"
			...
			<input
				...
				required
				class="input-base file:mr-3 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-sm file:bg-primary-50 file:text-primary-700 dark:file:bg-stone-800 dark:file:text-primary-300 hover:file:bg-primary-100 dark:hover:file:bg-stone-700"
			/>
			<div>
	...
	<!-- Version -->
	<div class="text-center text-xs text-stone-400 dark:text-stone-500 pt-2">
