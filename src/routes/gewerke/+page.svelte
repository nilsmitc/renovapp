...
<div class="space-y-6">
	...
	{#if gewerkeLokal.length > 0}
		<div class="grid grid-cols-1 sm:grid-cols-3 gap-4 stagger">
			...
			<div class="kpi-card animate-in">
				<div class="flex items-center gap-1.5 text-xs font-medium {gesamtPct > 100 ? 'text-red-500 dark:text-red-400' : gesamtPct >= 80 ? 'text-yellow-500 dark:text-yellow-400' : 'text-stone-400 dark:text-stone-500'} uppercase tracking-wide">
				...
				<div class="text-xl font-bold font-mono mt-1 {gesamtPct > 100 ? 'text-red-600 dark:text-red-400' : gesamtPct >= 80 ? 'text-yellow-600 dark:text-yellow-400' : 'text-green-600 dark:text-green-400'}">{formatCents(data.gesamtIst)}</div>
	...
	{#if zeigeFormular}
		<div class="card animate-fade">
			<h2 class="mb-4 text-lg font-semibold text-stone-800 dark:text-stone-200">Neues Gewerk anlegen</h2>
			<form method="POST" action="?/add" use:enhance={({ formElement }) => {
				...
				<div>
					<label for="farbe" class="mb-1 block text-sm font-medium text-stone-700 dark:text-stone-300">Farbe</label>
					<div class="flex gap-3 items-center">
						<input type="color" name="farbe" id="farbe" value="#6B7280" class="h-10 w-14 rounded-lg border border-stone-300 dark:border-stone-600 cursor-pointer" />
						<span class="text-xs text-stone-400 dark:text-stone-500">Wird in Charts verwendet</span>
					</div>
				</div>
				<div class="md:col-span-2">
					<label class="flex items-center gap-2 cursor-pointer text-sm text-stone-600 dark:text-stone-300">
						<input type="checkbox" name="pauschal" class="rounded" />
						Sammelgewerk – kein Budget-Alarm
						<span class="text-xs text-stone-400 dark:text-stone-500">(z.B. Generalunternehmer)</span>
					</label>
				</div>
	...
	{#if gewerkeLokal.length === 0}
		<div class="card py-12 text-center">
			...
			<p class="mt-3 text-stone-500 dark:text-stone-400">Noch keine Gewerke angelegt</p>
	...
	{:else}
		<div class="space-y-3 stagger">
			{#each gewerkeLokal as gewerk, i (gewerk.id)}
				...
				<div
					class="card overflow-hidden transition-all duration-200 animate-in {isDragging ? 'opacity-50 scale-[0.98]' : ''} {isDragOver ? 'ring-2 ring-primary-400 ring-offset-2' : ''}"
					...
					{#if editId === gewerk.id}
						...
						<div class="p-4 animate-fade">
							<form method="POST" action="?/update" use:enhance={() => {
								...
								<div>
									<label class="mb-1 block text-sm font-medium text-stone-700 dark:text-stone-300">Farbe</label>
									<input type="color" name="farbe" value={gewerk.farbe} class="h-10 w-14 rounded-lg border border-stone-300 dark:border-stone-600 cursor-pointer" />
								</div>
								<div class="md:col-span-2">
									<label class="flex items-center gap-2 cursor-pointer text-sm text-stone-600 dark:text-stone-300">
							...
							<div class="flex items-center gap-3">
								...
								<div class="flex-1 min-w-0">
									<div class="flex items-center gap-2 flex-wrap">
										<div class="w-4 h-4 rounded-sm shrink-0 color-dot" style="background-color: {gewerk.farbe}"></div>
										<span class="font-semibold text-stone-900 dark:text-stone-100">{gewerk.name}</span>
										{#if gewerk.pauschal}
								...
								<div class="flex items-center gap-1 shrink-0">
									<button onclick={() => (editId = gewerk.id)} class="rounded p-1.5 text-stone-400 hover:bg-primary-50 dark:hover:bg-primary-900/30 hover:text-primary-600 dark:hover:text-primary-400" title="Bearbeiten">
								...
								<div class="mt-3">
									{#if !gewerk.pauschal && info.budget > 0}
										<div class="flex items-baseline gap-3 text-sm">
											<span class="text-stone-500 dark:text-stone-400">Budget: <span class="font-mono tabular-nums font-medium text-stone-700 dark:text-stone-300">{formatCents(info.budget)}</span></span>
											<span class="text-stone-300 dark:text-stone-600">·</span>
											<span class="text-stone-500 dark:text-stone-400">Ist: <span class="font-mono tabular-nums font-medium {pct > 100 ? 'text-red-600 dark:text-red-400' : pct >= 80 ? 'text-yellow-600 dark:text-yellow-400' : 'text-green-600 dark:text-green-400'}">{formatCents(info.ist)}</span></span>
											<span class="text-stone-300 dark:text-stone-600">·</span>
											<span class="font-mono tabular-nums text-sm font-medium {pct > 100 ? 'text-red-600 dark:text-red-400' : pct >= 80 ? 'text-yellow-600 dark:text-yellow-400' : 'text-green-600 dark:text-green-400'}">{pct}%</span>
										</div>
										<div class="mt-1.5 h-1.5 w-full rounded-full bg-stone-100 dark:bg-stone-800 overflow-hidden">
									...
									{:else if !gewerk.pauschal}
										<span class="text-xs text-stone-400 dark:text-stone-500">Kein Budget gesetzt</span>
										{#if info.ist > 0}
											<span class="text-xs text-stone-400 dark:text-stone-500 ml-2">· Ist: <span class="font-mono">{formatCents(info.ist)}</span></span>
										{/if}
									{/if}
								</div>
								...
								{#if info.anzahlBuchungen > 0 || info.anzahlAuftraege > 0}
									<div class="mt-2 flex gap-3 text-xs">
										{#if info.anzahlBuchungen > 0}
											<a href="/buchungen?gewerk={gewerk.id}" class="text-primary-600 dark:text-primary-400 hover:underline">{info.anzahlBuchungen} {info.anzahlBuchungen === 1 ? 'Buchung' : 'Buchungen'}</a>
										{/if}
										{#if info.anzahlAuftraege > 0}
											<a href="/rechnungen?gewerk={gewerk.id}" class="text-primary-600 dark:text-primary-400 hover:underline">{info.anzahlAuftraege} {info.anzahlAuftraege === 1 ? 'Auftrag' : 'Aufträge'}</a>
										{/if}
									</div>
								{/if}
							{/if}
