<script lang="ts">
	...
	function applyTheme() {
		if (!chart) return;
		const c = chartColors();
		chart.data.datasets[0].borderColor = c.cumulative;
		chart.data.datasets[0].backgroundColor = c.cumulative;
		chart.data.datasets[1].borderColor = c.line;
		chart.data.datasets[1].backgroundColor = c.line;
		chart.data.datasets[2].borderColor = c.over;
	...
	onMount(() => {
		...
		chart = new Chart(chartCanvas, {
			...
			options: {
				responsive: true,
				plugins: {
					legend: { display: true, position: 'bottom', labels: { color: c.legend } },
					tooltip: {
						backgroundColor: c.tooltipBg,
						titleColor: c.tooltipText,
						bodyColor: c.tooltipText,
...
<div class="space-y-6">
	...
	{#if !data.keineDaten}
		{#if data.konfidenz === 'niedrig'}
			<div class="alert-warning flex gap-3">
		...
		{:else if data.konfidenz === 'mittel'}
			<div class="alert-info flex gap-3">
		...
		{:else}
			<div class="alert-success flex gap-3">
	...
	<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 stagger">
		...
		<div class="kpi-card animate-in">
			...
			<div class="text-xs text-stone-400 dark:text-stone-500 mt-1">{data.gesamtBudget > 0 ? Math.round((data.gesamtIst / data.gesamtBudget) * 100) : 0}% des Budgets</div>
		</div>
		...
		<div class="kpi-card animate-in">
			...
			<div class="text-xl font-bold font-mono mt-1 kpi-value {data.freiVerfuegbar < 0 ? 'text-red-600 dark:text-red-400' : ''}">{formatCents(data.freiVerfuegbar)}</div>
			<div class="text-xs text-stone-400 dark:text-stone-500 mt-1">Budget abzgl. aller Kosten</div>
		</div>
		...
		<div class="kpi-card animate-in">
			...
			{#if data.erschoepfungsDatum && data.restBudget > 0}
				...
				<div class="text-xs text-stone-400 dark:text-stone-500 mt-1">in ca. {data.restMonate} {data.restMonate === 1 ? 'Monat' : 'Monaten'}</div>
				{#if data.bekannteZahlungenGesamt > 0}
					<div class="mt-1 text-xs text-primary-500 dark:text-primary-400">inkl. {formatCents(data.bekannteZahlungenGesamt)} geplanter Abschläge</div>
				{/if}
			{:else if data.restBudget <= 0}
				<div class="text-xl font-bold text-red-600 dark:text-red-400 mt-1 kpi-value">Überschritten</div>
				<div class="text-xs text-red-400 dark:text-red-400/80 mt-1">Budget bereits überzogen</div>
			{:else if data.keineDaten}
				<div class="text-sm text-stone-400 dark:text-stone-500 mt-1">Noch keine Daten</div>
			{:else}
				<div class="text-xl font-bold text-green-600 dark:text-green-400 mt-1 kpi-value">Im Rahmen</div>
				<div class="text-xs text-stone-400 dark:text-stone-500 mt-1">Budget reicht laut Prognose</div>
			{/if}
		</div>
	</div>
	...
	<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 stagger">
		...
		<div class="kpi-card animate-in">
			...
			{#if data.burnRateMonatlich > 0}
				<div class="text-xl font-bold font-mono mt-1 kpi-value">{formatCents(data.burnRateMonatlich)}<span class="text-sm font-normal text-stone-400 dark:text-stone-500"> / Monat</span></div>
				<div class="text-xs text-stone-400 dark:text-stone-500 mt-1">Ø letzte {data.burnRateBasis} {data.burnRateBasis === 1 ? 'Monat' : 'Monate'}{#if data.teilmonatAusgaben > 0} · lfd. Monat: {formatCents(data.teilmonatAusgaben)}{/if}</div>
			{:else}
				<div class="text-sm text-stone-400 dark:text-stone-500 mt-1">Noch keine Daten</div>
			{/if}
		</div>
		...
		<div class="kpi-card animate-in">
			...
			<div class="text-xl font-bold font-mono mt-1 kpi-value {data.gesamtOffen > 0 ? 'text-orange-600 dark:text-orange-400' : ''}">{formatCents(data.gesamtOffen)}</div>
			{#if data.gesamtOffen > 0}
				<a href="/rechnungen" class="text-xs text-orange-500 dark:text-orange-400 hover:underline mt-1 block">Aufträge ansehen</a>
			{:else}
				<div class="text-xs text-stone-400 dark:text-stone-500 mt-1">Keine offenen Abschläge</div>
			{/if}
		</div>
		...
		<div class="kpi-card animate-in">
			...
			<div class="text-xl font-bold font-mono mt-1 kpi-value {data.gesamtRestauftrag > 0 ? 'text-violet-600 dark:text-violet-400' : ''}">{formatCents(data.gesamtRestauftrag)}</div>
			<div class="text-xs text-stone-400 dark:text-stone-500 mt-1">Vertraglich gebunden, nicht fakturiert</div>
		</div>
	...
	{:else}
		<div class="card p-4">
			<h2 class="text-sm font-semibold text-stone-700 dark:text-stone-300 mb-4">Ausgabenverlauf &amp; Prognose</h2>
			<canvas bind:this={chartCanvas}></canvas>
			<div class="mt-3 text-xs text-stone-400 dark:text-stone-500 text-center">
				Türkise Linie: tatsächliche Ausgaben &middot; Terrakotta gestrichelt: Prognose bei aktueller Burn Rate &middot; Rot gestrichelt: Gesamtbudget
			</div>
		</div>
	{/if}
	...
	{#if data.naechsteZahlungen.length > 0}
		<div class="card overflow-x-auto">
			<div class="px-4 py-3 border-b border-stone-100 dark:border-stone-800 bg-stone-50/80 dark:bg-stone-800/40 rounded-t-xl">
				...
				<p class="text-xs text-stone-400 dark:text-stone-500 mt-0.5">Offene Rechnungen chronologisch nach Fälligkeit</p>
			</div>
			<table class="w-full">
				<thead>
					<tr class="thead-row">
				...
				<tbody>
					{#each data.naechsteZahlungen as z (z.rechnungId + '-' + z.nummer)}
						<tr class="border-b border-stone-100 dark:border-stone-800 last:border-b-0 hover:bg-stone-50/50 dark:hover:bg-stone-800/50 transition-colors {z.effektivStatus === 'ueberfaellig' ? 'bg-red-50/50 dark:bg-red-950/30' : z.effektivStatus === 'bald_faellig' ? 'bg-amber-50/50 dark:bg-amber-950/30' : ''}">
							<td class="px-4 py-3 text-sm">
								{#if z.faelligkeitsdatum}
									<div>{formatDatum(z.faelligkeitsdatum)}</div>
									{#if z.tageVerbleibend !== null}
										{#if z.tageVerbleibend < 0}
											<div class="text-xs text-red-600 dark:text-red-400 font-medium">überfällig seit {Math.abs(z.tageVerbleibend)} Tagen</div>
										{:else if z.tageVerbleibend === 0}
											<div class="text-xs text-red-600 dark:text-red-400 font-medium">heute fällig</div>
										{:else if z.tageVerbleibend <= 7}
											<div class="text-xs text-amber-600 dark:text-amber-400 font-medium">in {z.tageVerbleibend} Tagen</div>
										{:else}
											<div class="text-xs text-stone-400 dark:text-stone-500">in {z.tageVerbleibend} Tagen</div>
										{/if}
									{/if}
								{:else}
									<span class="text-stone-400 dark:text-stone-500 text-xs italic">Kein Datum</span>
								{/if}
							</td>
							<td class="px-4 py-3 text-sm">
							...
							<td class="px-4 py-3 text-sm">
								<div class="flex items-center gap-1.5">
									<div class="w-2.5 h-2.5 rounded-full shrink-0 color-dot" style="background-color: {z.gewerkFarbe}"></div>
									<span class="text-stone-600 dark:text-stone-300">{z.gewerkName}</span>
								</div>
							</td>
						</tr>
					{/each}
	...
	{#if data.gewerkUebersicht.length > 0}
		<div class="card overflow-x-auto">
			<div class="px-4 py-3 border-b border-stone-100 dark:border-stone-800 bg-stone-50/80 dark:bg-stone-800/40 rounded-t-xl">
				<h2 class="text-sm font-semibold text-stone-700 dark:text-stone-300">Übersicht nach Gewerk</h2>
				<p class="text-xs text-stone-400 dark:text-stone-500 mt-0.5">Budget · Bezahlt · Offene Rechnungen · Restauftrag · Frei = was wirklich übrig ist</p>
			</div>
			<table class="w-full">
				<thead>
					<tr class="thead-row">
				...
				<tbody>
					{#each data.gewerkUebersicht as g (g.gewerk.id)}
						...
						<tr class="border-b border-stone-100 dark:border-stone-800 last:border-b-0 hover:bg-stone-50/50 dark:hover:bg-stone-800/50 transition-colors {!hatWerte && g.budget === 0 ? 'opacity-40' : ''}">
							<td class="px-4 py-3 text-sm">
								<div class="flex items-center gap-2">
									<div class="w-3 h-3 rounded-full shrink-0 color-dot" style="background-color: {g.gewerk.farbe}"></div>
									<span class="{hatWerte || g.budget > 0 ? 'font-medium' : 'italic text-stone-400 dark:text-stone-500'}">{g.gewerk.name}</span>
								</div>
							</td>
							<td class="px-4 py-3 text-sm text-right font-mono tabular-nums text-stone-500 dark:text-stone-400">
								{g.budget > 0 ? formatCents(g.budget) : '—'}
							</td>
							<td class="px-4 py-3 text-sm text-right font-mono tabular-nums">
								{g.ist > 0 ? formatCents(g.ist) : '—'}
							</td>
							<td class="px-4 py-3 text-sm text-right font-mono tabular-nums {g.offen > 0 ? 'text-orange-600 dark:text-orange-400 font-medium' : 'text-stone-300 dark:text-stone-600'}">
								{g.offen > 0 ? formatCents(g.offen) : '—'}
							</td>
							<td class="px-4 py-3 text-sm text-right font-mono tabular-nums {g.restauftrag > 0 ? 'text-violet-600 dark:text-violet-400 font-medium' : 'text-stone-300 dark:text-stone-600'}">
								{g.restauftrag > 0 ? formatCents(g.restauftrag) : '—'}
							</td>
							<td class="px-4 py-3 text-sm text-right font-mono tabular-nums">
								{#if g.budget > 0 || hatWerte}
									<span class="{g.frei < 0 ? 'text-red-600 dark:text-red-400 font-medium' : 'text-green-600 dark:text-green-400'}">{formatCents(g.frei)}</span>
								{:else}
