<script lang="ts">
	...
	function gewerkFarbe(id: string): string {
		return data.gewerke.find((g) => g.id === id)?.farbe ?? '#78716c';
	}
...
<div class="space-y-6">
	...
	<!-- Filters -->
	<div class="flex flex-wrap gap-3 card p-3">
		<div class="relative flex-1 min-w-48">
			<svg class="absolute left-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-stone-400 dark:text-stone-500 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" /></svg>
			<input
				type="search"
		...
		<div class="flex items-center gap-1 rounded-lg border border-stone-200 dark:border-stone-700 bg-stone-100 dark:bg-stone-800 p-1">
			{#each [['', 'Alle'], ['direkt', 'Direkt'], ['rechnung', 'Aus Auftrag'], ['lieferung', 'Aus Lieferung']] as [val, label]}
				<button
					onclick={() => applyFilter('herkunft', val)}
					class="rounded px-2.5 py-1 text-xs font-medium transition-colors {data.filter.herkunft === val || (!data.filter.herkunft && val === '') ? 'bg-white dark:bg-stone-700 text-primary-700 dark:text-primary-300 shadow-sm' : 'text-stone-500 dark:text-stone-400 hover:text-stone-800 dark:hover:text-stone-200'}"
				>{label}</button>
			{/each}
		</div>
	</div>
		...
		<table class="w-full">
			...
			<tbody>
					...
					<tr class="border-b border-stone-100 dark:border-stone-800 last:border-b-0 hover:bg-stone-50/50 dark:hover:bg-stone-800/50 transition-colors">
						...
						<td class="px-4 py-3 text-sm">
							<div class="flex items-center gap-1.5 flex-wrap">
								...
								{#if buchung.belege?.length}
									<a href="/buchungen/{buchung.id}" class="text-stone-400 dark:text-stone-500 hover:text-primary-500 dark:hover:text-primary-400 transition-colors" title="{buchung.belege.length} Beleg(e)">
								...
								{#if buchung.bezahltam && buchung.bezahltam !== buchung.datum}
									<span class="inline-flex items-center gap-0.5 rounded bg-emerald-50 px-1.5 py-0.5 text-xs font-medium text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300">
							...
							{#if buchung.taetigkeit}
								<div class="text-xs text-stone-400 dark:text-stone-500 mt-0.5 italic">{buchung.taetigkeit}</div>
							{:else if buchung.rechnungsreferenz}
								<div class="text-xs text-stone-400 dark:text-stone-500 mt-0.5">{buchung.rechnungsreferenz}</div>
							{/if}
						...
						<td class="px-4 py-3 text-sm">
							<div class="flex items-center gap-1.5">
								<div class="w-2.5 h-2.5 rounded-full flex-shrink-0 color-dot" style="background-color: {gewerkFarbe(buchung.gewerk)}"></div>
								{gewerkName(buchung.gewerk)}
							</div>
						</td>
						<td class="px-4 py-3 text-sm text-stone-600 dark:text-stone-300">{raumName(buchung.raum)}</td>
						<td class="px-4 py-3 text-sm text-stone-600 dark:text-stone-300">{buchung.kategorie}</td>
						<td class="px-4 py-3 text-sm text-right font-mono tabular-nums {buchung.betrag < 0 ? 'text-red-600 dark:text-red-400' : ''}">
							{formatCents(buchung.betrag)}
							{#if buchung.betrag < 0}
								<span class="badge-danger ml-1">Rückbuchung</span>
							{/if}
						</td>
						<td class="px-4 py-3 text-sm text-right">
							<a href="/buchungen/{buchung.id}" class="inline-flex items-center gap-1 text-primary-600 dark:text-primary-400 hover:underline font-medium">
				...
				{#if data.buchungen.length === 0}
					<tr><td colspan="7" class="px-4 py-12 text-center text-stone-400 dark:text-stone-500 text-sm">
	...
	{#if data.buchungen.length > 0}
		<div class="text-sm text-stone-500 dark:text-stone-400 text-right">
