...
<div class="space-y-6">
	<div class="flex items-center justify-between">
		...
		<form method="POST" action="?/delete" use:enhance>
			<button type="submit" class="inline-flex items-center gap-1 text-red-600 dark:text-red-400 text-sm hover:underline"
	...
	{#if data.buchung.belege.length > 0}
		<div class="card p-4">
			<h3 class="text-sm font-medium text-stone-700 dark:text-stone-300 mb-2">Belege verwalten</h3>
			<div class="space-y-2">
				{#each data.buchung.belege as beleg}
					<div class="flex items-center justify-between py-1">
						<a href="/belege/{data.buchung.id}/{beleg}" target="_blank" class="text-primary-600 dark:text-primary-400 hover:underline text-sm">{beleg}</a>
						<form method="POST" action="?/deleteBeleg" use:enhance>
							<input type="hidden" name="dateiname" value={beleg} />
							<button type="submit" class="inline-flex items-center gap-1 text-red-600 dark:text-red-400 text-xs hover:underline"
