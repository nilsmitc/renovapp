...
{#if error}
	<div class="alert-danger flex items-center gap-2 mb-4">
...
<form method="POST" action={action} use:enhance enctype="multipart/form-data" class="card p-6 space-y-4">
	<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
		<div>
			<label for="datum" class="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Datum</label>
		...
		<div>
			<label for="betrag" class="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Betrag (EUR)</label>
			...
			<div class="flex items-center gap-2 mt-1.5">
				...
				<label for="rueckbuchung" class="text-sm text-stone-700 dark:text-stone-300">Rückbuchung / Gutschrift</label>
			</div>
		</div>

		<div>
			<label for="gewerk" class="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Gewerk</label>
		...
		<div>
			<label for="raum" class="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Ort <span class="text-stone-400 dark:text-stone-500">(optional)</span></label>
		...
		<div>
			<label for="kategorie" class="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Kategorie</label>
		...
		<div>
			<label for="rechnungsreferenz" class="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Rechnungsreferenz <span class="text-stone-400 dark:text-stone-500">(optional)</span></label>
	...
	<div>
		<label for="beschreibung" class="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Beschreibung</label>
	...
	<div>
		<label for="taetigkeit" class="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">
			Tätigkeit <span class="text-stone-400 dark:text-stone-500 font-normal">(optional)</span>
		</label>
		{#if selectedGewerk?.pauschal}
			<p class="text-xs text-primary-600 dark:text-primary-400 mb-1.5">Sammelgewerk – Tätigkeit angeben für spätere Aufschlüsselung.</p>
		{/if}
	...
	<div class="border-t border-stone-100 dark:border-stone-800 pt-4">
		<div class="flex items-center gap-2">
			...
			<label for="bezahltam-toggle" class="text-sm font-medium text-stone-700 dark:text-stone-300">Als bezahlt markieren</label>
		...
		{#if showBezahltam}
			<div class="mt-2">
				<label for="bezahltam" class="block text-sm font-medium text-stone-600 dark:text-stone-400 mb-1">Bezahlt am</label>
	...
	{#if lieferanten.length > 0}
		<div class="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-stone-100 dark:border-stone-800 pt-4">
			<div>
				<label for="lieferant-select" class="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">
					Lieferant <span class="text-stone-400 dark:text-stone-500 font-normal">(optional)</span>
				</label>
				<select
					id="lieferant-select"
					class="input-base"
				...
				<p class="text-xs text-stone-400 dark:text-stone-500 mt-1">Filtert die Lieferungs-Auswahl</p>
			</div>

			<div>
				<label for="lieferungId" class="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">
					Lieferung <span class="text-stone-400 dark:text-stone-500 font-normal">(optional)</span>
				</label>
				...
				{#if selectedLieferantId && gefilterteLieferungen.length === 0}
					<p class="text-xs text-amber-600 dark:text-amber-400 mt-1">Noch keine Lieferungen für diesen Lieferanten</p>
				{/if}
			</div>
		</div>
	{/if}
	...
	<!-- Belege -->
	<div>
		<label for="belege" class="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Belege <span class="text-stone-400 dark:text-stone-500">(optional)</span></label>
		<input type="file" name="belege" id="belege" multiple accept=".pdf,.jpg,.jpeg,.png"
			class="input-base file:mr-3 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-sm file:bg-stone-100 file:text-stone-700 hover:file:bg-stone-200 dark:file:bg-stone-700 dark:file:text-stone-200 dark:hover:file:bg-stone-600" />
		<p class="text-xs text-stone-400 dark:text-stone-500 mt-1">PDF, JPG oder PNG</p>
	</div>

	<!-- Vorhandene Belege (Edit-Modus) -->
	{#if belege.length > 0 && buchungId}
		<div>
			<div class="text-sm font-medium text-stone-700 dark:text-stone-300 mb-2">Vorhandene Belege</div>
			<div class="flex flex-wrap gap-2">
				{#each belege as beleg}
					<div class="flex items-center gap-2 bg-stone-50 dark:bg-stone-800/50 border border-stone-200 dark:border-stone-700 rounded-lg px-3 py-1.5 text-sm">
						{#if isPdf(beleg)}
							<svg class="w-4 h-4 text-red-500 dark:text-red-400" fill="currentColor" viewBox="0 0 20 20"><path d="M4 18h12a2 2 0 002-2V6l-4-4H4a2 2 0 00-2 2v12a2 2 0 002 2zm2-10h2v4H6V8zm3 0h2v4H9V8zm3 0h2v4h-2V8z"/></svg>
						{:else}
							<svg class="w-4 h-4 text-primary-500 dark:text-primary-400" fill="currentColor" viewBox="0 0 20 20"><path d="M4 3a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V5a2 2 0 00-2-2H4zm12 12H4l4-8 3 6 2-4 3 6z"/></svg>
						{/if}
						<a href="/belege/{buchungId}/{beleg}" target="_blank" rel="noopener noreferrer" class="text-primary-600 dark:text-primary-400 hover:underline">{beleg}</a>
					</div>
				{/each}
			</div>
		</div>
