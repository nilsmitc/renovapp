	...
	const slides = [
		{
			...
			farbe: 'text-primary-600 dark:text-primary-400',
		...
		},
		{
			...
			farbe: 'text-stone-700 dark:text-stone-300',
		...
		},
		{
			...
			farbe: 'text-primary-700 dark:text-primary-300',
			bg: 'bg-primary-200 dark:bg-primary-800/40',
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4">
	<div class="bg-white dark:bg-stone-900 rounded-2xl shadow-2xl max-w-lg w-full p-8 relative">
		<!-- Slide content -->
		<div class="text-center transition-opacity duration-200 {visible ? 'opacity-100' : 'opacity-0'}">
			...
			<!-- Titel -->
			<h2 class="text-xl font-bold text-stone-900 dark:text-stone-100 mb-3">{slides[step].titel}</h2>

			<!-- Text -->
			<p class="text-sm text-stone-600 dark:text-stone-300 leading-relaxed mb-6">{slides[step].text}</p>
		</div>
		...
		<div class="flex items-center justify-center gap-2 mb-6">
			<div class="flex gap-1.5">
				{#each slides as _, i}
					<button
						onclick={() => goTo(i)}
						class="w-2.5 h-2.5 rounded-full transition-colors {i === step ? 'bg-primary-600' : 'bg-stone-300 dark:bg-stone-600 hover:bg-stone-400 dark:hover:bg-stone-500'}"
						aria-label="Slide {i + 1}"
					></button>
				{/each}
			</div>
			<span class="text-xs text-stone-400 dark:text-stone-500 ml-2">{step + 1} von {slides.length}</span>
		</div>

		<!-- Navigation -->
		<div class="flex items-center justify-between">
			<button onclick={onClose} class="text-sm text-stone-400 dark:text-stone-500 hover:text-stone-600 dark:hover:text-stone-300 transition-colors">
				Überspringen
			</button>
